 ...
 *
 * Tests cover:
 * - Circular buffer operation (288 samples, 5-minute intervals)
...
/**
 * Test 2.1: Buffer size is 288
 */
void test_buffer_size_288(void) {
    HistoryManager* history = HistoryManager::getInstance();

    // Add exactly 288 points
    for (int i = 0; i < 288; i++) {
        history->addDataPoint(22.0f, 50.0f, 1013.25f, 50);
    }

    TEST_ASSERT_EQUAL_INT(288, history->getDataPointCount());
}

/**
 * Test 2.2: Buffer wraparound
 * Verify that adding more than 288 points doesn't exceed buffer size
 */
void test_buffer_wraparound(void) {
    HistoryManager* history = HistoryManager::getInstance();

    // Add 300 points (exceeds buffer size)
    for (int i = 0; i < 300; i++) {
        history->addDataPoint(20.0f + (i % 10), 50.0f, 1013.25f, i % 100);
    }

    // Count should be capped at 288
    TEST_ASSERT_EQUAL_INT(288, history->getDataPointCount());
}
...
void test_old_data_overwritten(void) {
    ...
    // Add one more point (should overwrite oldest)
    ntp.setMockTime(2000);
    history->addDataPoint(25.0f, 60.0f, 1015.0f, 100);

    // Count should still be 288
    TEST_ASSERT_EQUAL_INT(288, history->getDataPointCount());

    // Export to JSON and verify newest data
    ...
    // Last temperature should be 25.0
...
void test_json_export_metadata(void) {
    ...
    TEST_ASSERT_EQUAL_INT(288, doc["maxSize"]);
}
...
void test_exactly_fill_buffer(void) {
    ...
    JsonArray temps = doc["temperatures"];
    TEST_ASSERT_EQUAL_INT(288, temps.size());
}
...
void test_overfill_buffer_by_one(void) {
    ...
    // Last value should be the newest
    TEST_ASSERT_FLOAT_WITHIN(0.1f, 25.0f, temps[temps.size() - 1]);
}
...
void test_multiple_wraparounds(void) {
    ...
    TEST_ASSERT_EQUAL_INT(288, doc["count"]);
}
